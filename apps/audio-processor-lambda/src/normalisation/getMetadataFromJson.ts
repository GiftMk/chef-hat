import { type Result, success, failure } from '@chef-hat/ts-result'
...
export const getMetadataFromJson = (json: {
	...
	try {
		...
		if (e instanceof Error) {
			return failure(
				`Failed to convert JSON ${JSON.stringify(json, null, 2)} into Loudnorm metadata. The following error occurred ${e.message}`,
			)
		}
		return failure(
			`Failed to convert JSON ${JSON.stringify(json, null, 2)} into Loudnorm metadata. An unknown error occurred`,
		)
	}
}
